"""Evaluate the dependency-free keyword baseline against the MVP dataset."""
...
def evaluate(top_k: int = 5) -> dict[str, float | int]:
    ...
    for case in cases:
        ...
        results = search_keyword(chunks, case["question"], top_k=top_k)
...
def main() -> None:
    metrics = evaluate()
    print("keyword_baseline_metrics")
