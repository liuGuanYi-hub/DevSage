...
该入口会串行执行评估、离线 JSON/Markdown 评估报告、backend unittest、pytest、MCP、可选 LangGraph、前端构建、一键演示启动、本地 HTTP smoke、actor capability smoke 和 Compose dry-run；当前环境输出 `steps=18`。报告写入 `evaluation/reports/offline-baseline.json` 和 `evaluation/reports/offline-baseline.md`。真实 Docker、外部 Issue 和远程 Embedding 仍保持显式配置/授权边界。
