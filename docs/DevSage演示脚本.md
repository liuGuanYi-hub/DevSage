...
门禁会验证数据集、50 条评测、backend unittest、pytest、MCP、可选 LangGraph、前端构建、一键演示启动、本地 HTTP smoke、actor capability smoke 和 Compose dry-run；当前环境输出 `steps=17`，不会执行 Docker `-Execute`。
