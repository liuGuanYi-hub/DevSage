...
在已有页面基础上，前端统一收敛为研发助手工作台：

1. **工作台**：左侧项目/导航，中间问题与自然语言答案，右侧引用证据和执行上下文；
2. **知识库**：项目卡片、数据源、索引状态、文件/Chunk 变化、失败文件、只读路径和最近更新时间；
3. **检索实验室**：测试问题、关键词命中、向量候选、混合排序、Rerank、Top-K 和最低相关度阈值；
4. **任务记录**：已完成、运行中、失败、可恢复任务，支持过滤、重试、恢复和查看完整证据；
5. **评测中心**：Recall@5、MRR、Context、Grounding、答案延迟、缓存命中和失败案例趋势；
6. **设置与安全**：认证状态、项目成员、角色能力、模型/Embedding、Issue/MCP 连接和审计日志；
7. **后续操作抽屉**：知识笔记、代码 Diff、Issue 写回默认收起，只有用户主动操作时展开，保留 preview → approval → write 流程。

布局原则：答案优先、证据可追溯、调试可折叠、写回不打断阅读；桌面端使用三栏布局，窄屏使用抽屉和单列证据布局。
...
目标：让 DevSage 从“可验证的 Agentic RAG 页面”升级为“可持续使用的研发排查工作台”。

任务：

- [x] 完成工作台导航、三栏响应式布局和项目上下文栏；
- [x] 前端消费后端 SSE，展示真实 Agent 节点状态、取消、超时、断线、重试；
- [x] 接入任务记录页：按项目读取持久化任务摘要，支持查看答案/引用/执行步骤，并按角色显示可恢复任务操作；
- [ ] 将知识笔记、代码变更和 Issue 写回收拢到后续操作抽屉；
- [ ] 完成知识库管理页和索引变化/失败文件展示；
- [ ] 完成检索实验室和 Chunk 检查器；
- [x] 完成任务历史状态筛选、耗时排序和批量恢复/重新运行；批量管理不包含删除或清空，评测中心仍待完成；
- [ ] 增加答案反馈、引用纠错和人工确认后的评测集回流；
- [ ] 完成正式认证、成员持久化、组织级权限和审计日志页面；
- [ ] 补齐 GitHub CI、Issue/PR 模板、截图/GIF、版本发布和在线演示入口。

阶段 6 的每个页面必须同时提供：空状态、加载状态、超时状态、错误重试状态、只读权限状态和证据不足状态。
