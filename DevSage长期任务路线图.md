...
> 推进研发助手工作台升级：当前已完成导航、项目上下文栏和三栏响应式骨架；下一阶段接入真实 SSE 进度、取消、超时、重试和任务恢复，同时保持现有离线 Graph、项目边界、证据引用和审批安全合同不变。
...
- [ ] 消费后端真实 SSE 事件，展示分类、检索、证据检查、答案生成等真实节点状态；
- [ ] 增加任务取消、超时、断线提示、重试和已持久化任务恢复入口；
