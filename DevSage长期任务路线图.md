...
> 推进研发助手工作台升级：导航、项目上下文栏、三栏响应式骨架、真实 SSE 进度/取消/重试链路、任务记录页、状态筛选、耗时排序和批量恢复/重跑已完成；下一阶段补后续操作抽屉，同时保持现有离线 Graph、项目边界、证据引用和审批安全合同不变。
...
- [x] 增加任务状态筛选、按更新时间/耗时排序、多选和批量恢复/重新运行；批量操作不提供删除或清空，避免误删审计记录；
- [ ] 建立评测中心，展示 Recall@5、MRR、Context、Grounding、延迟、缓存命中和失败案例；
- [x] 增加答案反馈和引用纠错入口，经过人工确认后再进入评测集；反馈保存到 DevSage `data/feedback/`，确认案例自动回流评测脚本；
- [ ] 完成正式认证、成员持久化、组织级权限与审计日志页面；
- [x] 补齐 GitHub 仓库 CI、Issue/PR 模板、截图/GIF 和版本发布入口；公网在线演示仍待部署，当前明确提供本地演示入口。

### 本阶段执行顺序

1. 工作台导航与三栏响应式骨架；
2. 真实 SSE 进度、取消、超时、重试与任务恢复（已完成，任务记录筛选、耗时排序和批量恢复/重跑也已完成）；
3. 知识库管理和检索实验室；
4. 任务记录、评测中心和反馈闭环；
